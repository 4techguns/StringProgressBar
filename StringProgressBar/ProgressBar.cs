...
namespace StringProgressBar
{
    ...
    {
        public static ProgressInfo SplitBar(double progress, double limit, int size = 40, char line = '▬', char filled = '•')
        {
            if (progress > limit)
            {
            ...
            else
            {
                ...
                string progressText = new string(line, (int)rounded);

                Regex expression = new Regex(".$");
                progressText = expression.Replace(progressText, filled.ToString());
